  ...
  server = proxy.createServer(
    {
      ssl: {
        ...
        cert: fs.readFileSync(
          process.env.KEYSTORE + "fochlac_com_cert_chain.pem"
        )
      },
      secure: true
    },
...
const handleServerError = (err, req, res) => {
  ...
  handleHttpsRequest = (req, res) => {
    ...
    if (hostData) {
      if (hostData.redirect) {
        res.writeHead(302, {
          Location: "https://" + hostData.redirectUrl + req.url
        });
        res.end();
      } else {
        ...
        server.web(
          ...
          {
            target:
              (hostData.http ? "http" : "https") +
              "://localhost:" +
              (typeof hostData.port === "string"
                ? process.env[hostData.port]
                : hostData.port) +
              hostData.url
          },
          err => {
  ...
  handleHttpRequest = (req, res) => {
    res.writeHead(302, {
      Location:
        "https://" +
        (urlMap[req.headers.host] && urlMap[req.headers.host].redirect
          ? urlMap[req.headers.host].redirectUrl
          : req.headers.host) +
        req.url
    });
    res.end();
